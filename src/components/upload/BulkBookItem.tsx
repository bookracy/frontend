      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center">
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs text-muted-foreground">
            {(book.file.size / 1024 / 1024).toFixed(2)} MB, {book.book_filetype || book.file.name.split(".").pop() || ""}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <Select value={book.book_filetype} onValueChange={(v) => onFieldChange("book_filetype", v)} disabled={isUploading}>
            ...
            <SelectContent>
              {["epub", "pdf", "mobi", "azw", "azw3", "fb2", "djvu"].map((type) => (
